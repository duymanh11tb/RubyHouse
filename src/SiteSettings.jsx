...
export default function SiteSettings({settings={}}){
  useEffect(()=>{
    ...
    const heading=document.querySelector('.hero-copy h1');if(heading){const renderHeading=()=>{const title=String(settings.hero_title||'Your trusted home').trim().split(/\s+/),accent=String(settings.hero_accent||'AWAY FROM HOME.').trim().split(/\s+/);const home=title.pop()||'',away=accent.shift()||'';heading.replaceChildren(document.createTextNode(title.join(' ')),document.createElement('br'),document.createTextNode(`${home} `),Object.assign(document.createElement('em'),{textContent:away}),document.createElement('br'),Object.assign(document.createElement('em'),{textContent:accent.join(' ')}));};renderHeading();headingObserver=new MutationObserver(()=>{if(heading.childNodes.length!==6||heading.querySelectorAll('em').length!==2)renderHeading();});headingObserver.observe(heading,{childList:true});}
